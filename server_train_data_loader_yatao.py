...
class ImageLoader(VisionDataset):
    def __init__(self, windowsize=128, test=False, datafile='', _num_smpls=4):
        self.wsize = windowsize
        super().__init__(None)
        self.num_smpls, self.sh_x, self.sh_y = _num_smpls, 10980, 10980  # size of each image
        # self.num_smpls, self.sh_x, self.sh_y = 1, 3980, 3980 # size of each image
...
##############################################################################
        ...
        RGB_normal[:,:,:3] = np.asarray(RGB_sample[:,:,:3], np.float32)/3000.0
